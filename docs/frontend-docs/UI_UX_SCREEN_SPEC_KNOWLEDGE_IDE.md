...
Mục đích: hỏi đáp trong workspace, có thể giới hạn nguồn bằng `@file` và `@folder`.

Layout:

- Chat session list theo workspace.
- Chat canvas.
- Source mention input trong composer.
- Source citation panel.

Source controls:

- Mặc định không chọn source: hỏi toàn workspace.
- Gõ `@` mở command palette chọn File hoặc Folder.
- `@file` hiển thị chip tên file trong input.
- `@folder` hiển thị chip tên folder và mặc định include subfolders.
- Có thể chọn nhiều file/folder; FE gửi `mentionedSources` theo message.

States:

- Không có tài liệu completed.
- Mention file/folder không còn tồn tại.
- Folder mention resolve ra 0 document completed.
- AI đang trả lời.
- Không tìm thấy nguồn liên quan.
- Câu hỏi tiếp theo giữ chat history, nhưng source mentions chỉ áp dụng cho message hiện tại trừ khi user pin lại trong UI.
