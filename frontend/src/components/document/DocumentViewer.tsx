...
function DocumentViewerContent({ 
  ...
  return (
    <div className={cn("flex flex-col flex-1 min-h-0", minimal ? "bg-surface-0" : "bg-background")}>
      {!minimal && <DocumentHeader document={document} viewMode={viewMode} setViewMode={setViewMode} />}
      ...
      <div className={cn("flex-1 flex flex-col min-h-0", viewMode === 'original' ? "overflow-hidden" : "overflow-y-auto")}>
        <div className={cn(
          ...
          viewMode === 'summary'
            ? "max-w-[1280px] gap-12 p-6 lg:p-10"
            : viewMode === 'extracted' || viewMode === 'chunks'
              ? "max-w-6xl p-5 lg:p-8"
              : ""
        )}>
