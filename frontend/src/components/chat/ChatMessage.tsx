          ...
          {!isUser && message.sources.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {message.sources.map((source, idx) => (
                <div 
                  key={idx}
                  className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-surface-100 border border-border text-xs text-surface-600 hover:bg-surface-200 cursor-pointer transition-colors"
                >
